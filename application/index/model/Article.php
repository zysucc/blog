...
class  Article extends Model
{
     ...
     * @return mixed
     */
    public function getOneTest($where = '')
    {
        $data=$this->where($where)->setInc('art_hit');
        //$data = $this->alias('a')->field('a.*,m.menu_name')->join('lt_menu m','m.menu_id = a.art_pid')->where($where)->find();
        return $data;
    }
    ...
    {
        $data = $this->field('art_id,art_hit,art_title')->where('art_pid','neq',99)->limit(8)->order('art_hit desc')->select();
        foreach ($data as $key =>$value){
            $config= config()['index_config']['redis_port'];
            $redismodel=new redismodel($config);
            $data[$key]['art_hit']=$redismodel->getValue($value['art_id']);
        }
        return $data;
    }
     ...
     * @return false|int
     */
    public function upHotArticle($art_id,$art_hit)
    {
        $data=['art_hit'=>$art_hit];
        $result = $this->allowField(true)->save($data,['art_id'=>$art_id]);
        return $result;
    }
}